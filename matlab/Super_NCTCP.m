...
for i=1:L
    ...
    in_k(i)=KK;
    k=in_k(i);
    HH{i}.H{1} =  rand( par.patsize*par.patsize, k );
    HH{i}.H{1} = HH{i}.H{1} / diag( sqrt( sum( HH{i}.H{1}.^2 ) ) );
    
    HH{i}.H{2} =  rand( length(ind{i}), k );
...
for i=1: maxIter
    ...
    for id =1 : L
        ...
        W1 = khatrirao( HH{id}.H{ [2 3] }, 'r' );
        P1 = khatrirao({HH{id}.H{2},HH{id}.D},'r');
        G1=(HH{id}.H{3}'*HH{id}.H{3}).*(HH{id}.H{2}'*HH{id}.H{2});
        PTP=(HH{id}.D'*HH{id}.D).*(HH{id}.H{2}'*HH{id}.H{2});
        %         G1= ones(k,k);
        %         for j = [2 3]
...
%         clear tt1 Mtt1;
        ...
        W2 = khatrirao( HH{id}.H{ [1 3] }, 'r' );
        P2 = khatrirao({HH{id}.H{1},HH{id}.D},'r');
        G2=(HH{id}.H{3}'*HH{id}.H{3}).*(HH{id}.H{1}'*HH{id}.H{1});
        P2TP2=(HH{id}.D'*HH{id}.D).*(HH{id}.H{1}'*HH{id}.H{1});
        %         G2 = ones(k,k);
        %         for jj= [1 3]
        ...
        leftA= R'*R;
        %         rightA=W3'*W3;
        rightA =(HH{id}.H{2}'*HH{id}.H{2}).*(HH{id}.H{1}'*HH{id}.H{1});
        ...
        tempPatch{id}=double(ktensor( {HH{id}.H{1},HH{id}.H{3},HH{id}.H{2}}));
