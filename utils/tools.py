...
def videoGaussian(X, s):
    X_copy = X.copy()
    for frames in range(X.shape[3]):
        for i in range(X.shape[2]):
            X_copy[:, :, i, frames] = convolve(X[:, :, i, frames], s, mode='reflect')
    return X_copy
# 下采样 downsample
...
def VideoDownsample(inMatrix, rate):
    inMatrix_ = inMatrix.copy()
    return inMatrix_[::rate, 0::rate, :, :]
...
def videoUpsample(V_Interpolated, ratio, frames):
    [r, c, b, d] = V_Interpolated.shape
    VL = np.zeros([ratio * r, ratio * c, b, d])
    for i in range(frames):
        I_Interpolated = V_Interpolated[:, :, :, i]
        L = 45
        [r, c, b] = I_Interpolated.shape
        kernel = ratio * signal.firwin(L, 1 / ratio)
        kernel = np.reshape(kernel, [1, len(kernel)])
